...
public class ServiceTracker<S, T, U extends ServiceEvent>
{
     ...
     *         invalid syntax.
     */
    private ServiceReference<S>[] getInitialReferences(String className, String filterString)
        throws InvalidSyntaxException
    {
        ServiceReference<S>[] result = ( ServiceReference<S>[] ) context.getServiceReferences( className,
            filterString );
        return result;
    }
            ...
            while ( true )
            {
                ...
                synchronized ( this )
                {
                    ...
                    if ( tracked.get( item ) != null )
                    {
                        ...
                        if ( DEBUG )
                        {
                            System.out.println( "AbstractTracked.trackInitial[already tracked]: " //$NON-NLS-1$
                                + item );
                        }
    ...
    private class Tracked extends AbstractTracked<ServiceReference<S>, T, U> implements ExtendedServiceListener<U>
    {
        ...
        final public void serviceChanged(final U event)
        {
            ...
            switch ( event.getType() )
            {
                ...
                case ServiceEvent.UNREGISTERING:
                    ...
                    break;
                default:
                    System.out.println( "Unrecognized event type: ServiceTracker.Tracked.serviceChanged["
        ...
        final void customizerAdded(final ServiceReference<S> item, final U related, final T object, int trackingCount,
            int serviceCount)
        {
         ...
         * @param trackingCount
         */
        final void customizerModified(final ServiceReference<S> item, final U related, final T object,
            int trackingCount)
        {
