...
class ScrManagedServiceMetaTypeProvider extends ScrManagedService implements MetaTypeProvider
{
    ...
    public ObjectClassDefinition getObjectClassDefinition(String id, String locale)
    {
        ...
        adList.add( new AttributeDefinitionImpl( ScrConfiguration.PROP_LOGLEVEL, "SCR Log Level",
            ...
            AttributeDefinition.INTEGER, new String[] { String.valueOf( this.getScrConfiguration().getLogLevel() ) }, 0,
            new String[] { "Debug", "Information", "Warnings", "Error" }, new String[] { "4", "3", "2", "1" } ) );
        ...
        adList.add(
            new AttributeDefinitionImpl( ScrConfiguration.PROP_DELAYED_KEEP_INSTANCES, "Keep Component Instances",
        ...
        adList.add( new AttributeDefinitionImpl( ScrConfiguration.PROP_LOCK_TIMEOUT, "Lock timeout milliseconds",
            "How long a lock is held before releasing due to suspected deadlock", AttributeDefinition.LONG,
            new String[] { String.valueOf( this.getScrConfiguration().lockTimeout() ) }, 0, null, null ) );

        adList.add( new AttributeDefinitionImpl( ScrConfiguration.PROP_STOP_TIMEOUT, "Stop timeout milliseconds",
            "How long stopping a bundle is waited for before continuing due to suspected deadlock",
            AttributeDefinition.LONG, new String[] { String.valueOf( this.getScrConfiguration().stopTimeout() ) }, 0,
            null, null ) );

        adList.add( new AttributeDefinitionImpl( ScrConfiguration.PROP_GLOBAL_EXTENDER, "Global Extender",
            "Whether to extend all bundles whether or not visible to this bundle.", false ) );
    ...
    private static class AttributeDefinitionImpl implements AttributeDefinition
    {
        ...
        {
            this( id, name, description, BOOLEAN, new String[] { String.valueOf( defaultValue ) }, 0, null, null );
        }
