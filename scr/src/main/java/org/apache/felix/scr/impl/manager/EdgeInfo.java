...
class EdgeInfo
{
    ...
    private void waitForLatch(AbstractComponentManager<?> m_componentManager, CountDownLatch latch,
        String componentName, String methodName, String latchName)
    {
        try
        {
            if ( !latch.await( m_componentManager.getLockTimeout(), TimeUnit.MILLISECONDS ) )
            {
                m_componentManager.log( LogService.LOG_ERROR, "DependencyManager : {0} : timeout on {1} latch {2}",
    ...
    {
        return open == -1 || trackingCount < open || ( close != -1 && trackingCount > close );
    }
