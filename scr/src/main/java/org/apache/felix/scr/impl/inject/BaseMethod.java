...
public abstract class BaseMethod<P, T>
{
    ...
    protected BaseMethod(final String methodName, final Class<?> componentClass, final DSVersion dsVersion, final boolean configurableServiceProperties)
    {
        this(methodName, methodName != null, componentClass, dsVersion,
            configurableServiceProperties);
    }

    protected BaseMethod(final String methodName, final boolean methodRequired, final Class<?> componentClass, final DSVersion dsVersion, final boolean configurableServiceProperties)
    {
    ...
    synchronized void setMethod(MethodInfo<T> methodInfo, SimpleLogger logger)
    {
        ...
        if (m_method != null)
        {
            ...
            logger.log(LogService.LOG_DEBUG, "Found {0} method: {1}",
                new Object[] { getMethodNamePrefix(), m_method }, null);
        }
        else if (m_methodRequired)
        {
            m_state = NotFound.INSTANCE;
            logger.log(LogService.LOG_ERROR,
                "{0} method [{1}] not found; Component will fail",
                new Object[] { getMethodNamePrefix(), getMethodName() }, null);
        }
        else
        {
            // optional method not found, log as DEBUG and ignore
            logger.log(LogService.LOG_DEBUG, "{0} method [{1}] not found, ignoring",
                new Object[] { getMethodNamePrefix(), getMethodName() }, null);
            m_state = NotApplicable.INSTANCE;
        }
    }
        ...
        while (true)
        {
            ...
            catch (SuitableMethodNotAccessibleException ex)
            {
                ...
                logger.log(LogService.LOG_ERROR,
                    ...
                    new Object[] { getMethodName(), theClass.getName(),
                            targetClass.getName() },
                    null);
                break;
            }
    ...
    protected abstract MethodInfo<T> doFindMethod(final Class<?> targetClass,
        final boolean acceptPrivate, final boolean acceptPackage, SimpleLogger logger)
        throws SuitableMethodNotAccessibleException, InvocationTargetException;

    private MethodResult invokeMethod(final Object componentInstance,
        final P rawParameter, SimpleLogger logger) throws InvocationTargetException
    {
        try
        {
            if (componentInstance != null)
            {
                ...
                logger.log(LogService.LOG_DEBUG, "invoking {0}: {1}: parameters {2}",
                    new Object[] { getMethodNamePrefix(), getMethodName(),
                            Arrays.asList(params) },
                    null);
                Object result = m_method.invoke(componentInstance, params);
                logger.log(LogService.LOG_DEBUG, "invoked {0}: {1}",
                    new Object[] { getMethodNamePrefix(), getMethodName() }, null);
                return new MethodResult((m_method.getReturnType() != Void.TYPE),
        ...
        catch (IllegalAccessException ex)
        {
            ...
            logger.log(LogService.LOG_DEBUG, "Method {0} cannot be called",
                new Object[] { getMethodName() }, ex);
        }
    ...
    public /* static */ Method getMethod(Class<?> clazz, String name,
        Class[] parameterTypes, boolean acceptPrivate, boolean acceptPackage,
        SimpleLogger logger)
        throws SuitableMethodNotAccessibleException, InvocationTargetException
    {
        try
        ...
        catch (NoSuchMethodException nsme)
        {
            ...
            if (logger.isLogEnabled(LogService.LOG_DEBUG))
            {
                String argList = (parameterTypes != null)
                    ? Arrays.asList(parameterTypes).toString() : "";
                logger.log(LogService.LOG_DEBUG, "Declared Method {0}.{1}({2}) not found",
                    new Object[] { clazz.getName(), name, argList }, null);
            }
        }
     ...
     */
    protected static boolean accept(final Method method, boolean acceptPrivate,
        boolean acceptPackage, boolean allowReturnValue)
    {
        if (!(Void.TYPE == method.getReturnType()
            || (ClassUtils.MAP_CLASS == method.getReturnType() && allowReturnValue)))
        {
            return false;
        }
        ...
        final MethodResult methodCallFailureResult, SimpleLogger logger)
    {
        ...
        {
            logger.log(LogService.LOG_ERROR, "The {0} method has thrown an exception",
                new Object[] { getMethodName() }, ite.getCause());
        }
    ...
    private static interface State
    {
        ...
        <P, T> MethodResult invoke(final BaseMethod<P, T> baseMethod,
            final Object componentInstance, final P rawParameter, SimpleLogger logger)
            throws InvocationTargetException;

        <P, T> boolean methodExists(final BaseMethod<P, T> baseMethod,
            SimpleLogger logger);
    }
    ...
    private static class NotApplicable implements State
    {
        ...
        public <P, T> MethodResult invoke(final BaseMethod<P, T> baseMethod,
            final Object componentInstance, final P rawParameter, SimpleLogger logger)
        {
            return MethodResult.VOID;
        }

        public <P, T> boolean methodExists(final BaseMethod<P, T> baseMethod,
            SimpleLogger logger)
        {
            return true;
        }
    ...
    private static class NotResolved implements State
    {
        ...
        {
            logger.log(
                LogService.LOG_DEBUG, "getting {0}: {1}", new Object[] {
            ...
            catch (InvocationTargetException ex)
            {
                logger.log(LogService.LOG_WARNING, "{0} cannot be found",
                    new Object[] { baseMethod.getMethodName() }, ex.getTargetException());
            }
            ...
            throws InvocationTargetException
        {
            ...
            return baseMethod.getState().invoke(baseMethod, componentInstance,
                rawParameter, logger);
        }
    ...
    private static class NotFound implements State
    {
            ...
            final Object componentInstance, final P rawParameter, SimpleLogger logger)
        {
            ...
            // method
            logger.log(LogService.LOG_ERROR,
                "{0} method [{1}] not found", new Object[] {
                        baseMethod.getMethodNamePrefix(), baseMethod.getMethodName() },
                null);
            return null;
        }
