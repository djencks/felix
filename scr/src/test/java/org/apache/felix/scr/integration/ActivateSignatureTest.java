...
public class ActivateSignatureTest extends ComponentTestBase
{
    ...
    public void test()
    {
        ...
        {
            TestCase.assertTrue(
                "Expecting component " + component.name + " to be enabled",
                component.defaultEnabled);

            ComponentConfigurationDTO cc = findComponentConfigurationByName(
                component.name, 0);
            TestCase.assertEquals(
                "Expecting component " + component.name + " to be active",
                ComponentConfigurationDTO.ACTIVE, cc.state);

            TestCase.assertNotNull("Expect activate method to be called",
                AbstractActivateSignatureTestComponent.getInstance(component.name));
