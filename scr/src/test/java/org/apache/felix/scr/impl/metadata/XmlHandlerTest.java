...
public class XmlHandlerTest extends TestCase
{
    ...
    public void test_unclosed_elements() throws Exception
    {
        try
        {
            readMetadataFromString(
                "<component name=\"n\"><implementation class=\"n\" /><component name=\"x\">");
    ...
    public void test_namespace_1_1_0_felix() throws Exception
    {
        ...
        assertEquals("1 Descriptor expected", 1, metadataList.size());
        final ComponentMetadata metadata = (ComponentMetadata) metadataList.get(0);
        assertEquals("Expect NS 1.1.0-felix", DSVersion.DS11Felix,
            metadata.getDSVersion());
    }
    ...
    public void test_namespace_1_2_0_felix() throws Exception
    {
        ...
        assertEquals("1 Descriptor expected", 1, metadataList.size());
        final ComponentMetadata metadata = (ComponentMetadata) metadataList.get(0);
        assertEquals("Expect NS 1.2.0-felix", DSVersion.DS12Felix,
            metadata.getDSVersion());
    }
    ...
    public void test_component_attributes_11() throws Exception
    {
        ...
        ComponentMetadataTest.failDS10Validation(
            (ComponentMetadata) metadataList10.get(0), "activate", logger);
        ComponentMetadataTest.failDS10Validation(
            (ComponentMetadata) metadataList10.get(1), "deactivate", logger);
        ComponentMetadataTest.failDS10Validation(
            (ComponentMetadata) metadataList10.get(2), "modified", logger);
        ComponentMetadataTest.failDS10Validation(
            (ComponentMetadata) metadataList10.get(3), "configuration-policy", logger);
        ...
        assertEquals("Expected Deactivate Method set", "mydeactivate",
            cm11.getDeactivate());
        assertTrue("Activate method expected to be declared",
            cm11.isDeactivateDeclared());
        assertEquals("Expected Modified Method set", "mymodified", cm11.getModified());
        assertEquals("Expected Configuration Policy set",
            ComponentMetadata.CONFIGURATION_POLICY_IGNORE, cm11.getConfigurationPolicy());
    }
    ...
    public void test_component_no_name() throws Exception
    {
        ...
        final ComponentMetadata cm11 = (ComponentMetadata) metadataList11.get(0);
        cm11.validate(logger);
        assertEquals("Expected name equals class", cm11.getImplementationClassName(),
            cm11.getName());
    }
    ...
    public void test_all_elements_10() throws Exception
    {
        ...
        assertEquals("component name", "components.all.factory",
            cm10.getFactoryIdentifier());
        ...
        assertTrue("Deactivate method expected to be declared",
            cm10.isDeactivateDeclared());
        assertEquals("configuration policy", "ignore", cm10.getConfigurationPolicy());

        // from the implementation element
        assertEquals("component name", "components.all.impl",
            cm10.getImplementationClassName());
        ...
        assertEquals("file.property value", "Property from File",
            file_property.getValue());
        ...
        assertEquals("ref.name target prop name", "ref.name.target",
            rm.getTargetPropertyName());
    ...
    {
        final List metadataList10 = readMetadata(
    ...
    {
        final List metadataList11 = readMetadata(
    ...
    private List readMetadata(String filename)
        throws IOException, ComponentException, XmlPullParserException, Exception
    {
    ...
    private PropertyMetadata getPropertyMetadata(final ComponentMetadata cm,
        final String name)
    {
